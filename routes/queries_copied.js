...
const MAP_TABLENAME_SEARCHKEY_FIELDS = {
	username : 1 
} 
const get_searchkey_fields = (tablename,searchkey)=>{
	let liker = convliker ( searchkey ) 
	switch (tablename){
		case 'username' : 
			return // { [Op.or] : 
				[ 
					{ uesrname : { [Op.like] : liker }}
...
router.get('/rows/:tablename/:fieldname/:fieldval/:offset/:limit/:orderkey/:orderval' , async (req,res)=>{
	...
	fieldexists(tablename, fieldname ).then(async resp=>{
		...
		if ( date0 && date1 ) { jfilter[ [Op.and] ] = [ 
				{ [Op.gte] : date0 }
			, { [Op.lte] : date1 }
			]
		} else {}
		if ( date1 ) { jfilter['createdat'] = { [Op.lte] : date1 }} else {}
		if ( date0 ) { jfilter['createdat'] = { [Op.gte] : date0 }} else {}

		if ( searchkey && MAP_TABLENAME_SEARCHKEY_FIELDS [ searchkey ] ) {
			jfilter[ [Op.or] ] = get_searchkey_fields ( tablename , searchkey )	
		}
		db[tablename].findAll ({raw:true
			, where : {... jfilter
