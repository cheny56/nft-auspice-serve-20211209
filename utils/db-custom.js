  ...
  return new Promise(async(resolve,reject)=>{
    ...
    findone('items', {itemid}).then(async respitem=>{
      ...
      aproms[aproms.length]=findone('users', { username: respitem.author } ) // 0 - author
			...
			aproms[aproms.length]=findall( 'itembalances' , { itemid , active : 1 } ) // 10 
...
const resolve_salestatus=(itemid, jfilter)=>{
	let aproms=[]
	return new Promise(async(resolve, reject)=>{
		if(itemid){}else{resolve(null)}
		findone('items', {itemid, ...jfilter}).then(async respitem=>{
			if(respitem && respitem.author) {} else {resolve(null);return}
			aproms[aproms.length] = findone('users', { username: respitem.author } )
			aproms[aproms.length] = findone('logfavorites', {itemid: respitem.itemid, username: respitem.author})
			...
			Promise.all(aproms).then(async aresps=>{
				...
				console.log(aresps[1])
				resolve({
					author: aresps[0]
					, item: respitem
					, ilikethisitem: aresps[1]
				})
			})
...
////////
const queryitemdata_user=async (itemid , username, jfilters={} ) =>{
  let aproms=[]
  return new Promise(async(resolve,reject)=>{
    if(itemid){} else {resolve(null)}
    findone('items', {itemid}).then(async respitem=>{
      ...
      aproms[aproms.length]=findone( 'users', { username: respitem.author } ) // 0 - author
			...
			aproms[aproms.length]=findall( 'itembalances' , { itemid , active : 1 } ) // 12
			//console.log()
